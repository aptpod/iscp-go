...
func TestTCPInfoProvider_StartStop(t *testing.T) {
	...
	t.Run("success: values remain accessible after stop", func(t *testing.T) {
		...
		// Verify default values are returned (since net.Pipe doesn't provide real TCP_CONNECTION_INFO)
		...
		// BytesInFlight is managed by kernel (Tcpi_snd_sbbytes), should be 0 for net.Pipe
...
// Note: Testing actual TCP_CONNECTION_INFO retrieval requires a real TCP connection,
