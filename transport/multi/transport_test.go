...
var _ StatusAwareTransport = (*mockTransport)(nil)
...
func TestMultiTransport_AddTransport(t *testing.T) {
	defer goleak.VerifyNone(t)

	t.Run("Successfully add and read from new transport", func(t *testing.T) {
		initialMock := newMockTransport("initialMock")
		transports := TransportMap{
			"initial": initialMock,
		}
		...
		// Add a new transport
		addedMock := newMockTransport("addedMock")
		err = mt.AddTransport("added", addedMock)
		require.NoError(t, err)

		// Test reading from the newly added transport
		testDataAdded := []byte("data from added transport")
		addedMock.readCh <- testDataAdded

		received, err := mt.Read()
		require.NoError(t, err)
		assert.Equal(t, testDataAdded, received, "Should read data from the newly added transport")

		// Test reading from the initial transport
		testDataInitial := []byte("data from initial transport")
		initialMock.readCh <- testDataInitial

		received, err = mt.Read()
		require.NoError(t, err)
		assert.Equal(t, testDataInitial, received, "Should still read data from the initial transport")

		// Write to current (initial) and check
		writeData := []byte("write test")
		err = mt.Write(writeData)
		require.NoError(t, err)
		select {
		case d := <-initialMock.writeCh:
			assert.Equal(t, writeData, d)
		case <-time.After(50 * time.Millisecond):
			t.Fatal("write to initial mock timed out")
		}
	})

	t.Run("Fail to add transport with duplicate ID", func(t *testing.T) {
		initialMock := newMockTransport("initialMock")
		transports := TransportMap{
			"initial": initialMock,
		}
		...
		// Attempt to add a new transport with an existing ID
		duplicateMock := newMockTransport("duplicateMock")
		err = mt.AddTransport("initial", duplicateMock)
		require.Error(t, err, "Should return an error when adding transport with duplicate ID")
