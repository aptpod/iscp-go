...
// reconnecting state.
//
// Root cause: selectMinTxBytes() calls reconnect.Transport.TxBytesCounterValue()
...
func TestByteBalancedSelector_Get_BlocksWhenTransportReconnecting(t *testing.T) {
	...
	trA, err := reconnect.Dial(reconnect.DialConfig{
		...
		PingInterval:         time.Hour, // Don't send pings during test
		ReadTimeout:          time.Hour, // Don't timeout during test
		Logger:               log.NewNop(),
	})
	...
	// Currently, Get() → selectMinTxBytes() → TxBytesCounterValue() → r.mu.Lock() → BLOCKS
	done := make(chan transport.TransportID, 1)
	go func() {
		done <- selector.Get(100)
	}()

	select {
	case id := <-done:
		// Expected behavior after fix: Get() returns quickly with connected transport
		assert.Equal(t, transport.TransportID("trA"), id,
	...
	case <-time.After(2 * time.Second):
		...
		t.Fatal("ByteBalancedSelector.Get() blocked for >2s when a transport is reconnecting - " +
			"this is caused by selectMinTxBytes() calling TxBytesCounterValue() which acquires " +
